...
/***/ (function(module, exports, __webpack_require__) {
	...
	*/
	...
	var Rtimer = function () {
	    function Rtimer(param) {
	        _classCallCheck(this, Rtimer);
	    ...
	    */
	    ...
	    _createClass(Rtimer, [{
	        key: 'EndDataUnix',
	        value: function EndDataUnix() {
	            var that = this;

	            var Data = that.Data;
	            var split = that.split;
	            var arrDate = Data.split(split);
	            ...
	            try {
	                for (var _iterator = arrDate.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	        ...
	        */
	        ...
	        key: 'CalculaterTime',
	        value: function CalculaterTime() {
	            var that = this;

	            var now = new Date().getTime();
	            var end = that.endUnix;

	            if (now - end > 0) {
	                that.cb();
	            } else {
	                var ele = that.ele;
	                var Mixtimer = end - now;

	                var D_Seconds = 1000 * 60 * 60 * 24;
	                var H_Seconds = 1000 * 60 * 60;
	                var M_Seconds = 1000 * 60;
	                var seconds = 1000;

	                var d = Math.floor(Mixtimer / D_Seconds);
	                var h = Math.floor((Mixtimer - d * D_Seconds) / H_Seconds);
	                var m = Math.floor((Mixtimer - d * D_Seconds - h * H_Seconds) / M_Seconds);
	                var s = Math.floor((Mixtimer - d * D_Seconds - h * H_Seconds - m * M_Seconds) / seconds);

	                var r = Array.from([d, h, m, s]);
	                ...
	                try {
	                    for (var _iterator2 = r.keys()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
